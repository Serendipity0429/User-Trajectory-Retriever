...
class MessageForm(forms.ModelForm):
    ...
    def clean_recipients(self):
        ...
        recipient_ids = [int(uid) for uid in recipient_ids_str.split(',') if uid.strip()]
    ...
    def clean(self):
        ...
        if not recipients and not send_to_all:
            raise ValidationError("You must select at least one recipient or check 'Send to all users'.")
