        ...
        if (trial.query_instruction && trial.query_instruction.includes('*** SYSTEM PROMPT ***')) {
            ...
            const userPart = parts[1] ? parts[1].trim() : '';
                            ...
                            if (systemPart) {
                                ...
                                chatContent += createMessageBubble('system', systemHtml, 'bg-light border-secondary border-opacity-10 shadow-none'); 
                            }                                if (userPart) {
                chatContent += createMessageBubble('user', userPart);
            }
        } else {
                let instr = trial.query_instruction || trial.instruction;
                if (!instr && trial.trial_number === 1) instr = questionText;
                if (instr) {
                chatContent += createMessageBubble('user', instr);
                }
        }
                    ...
                    if (trial.search_query) {
                        ...
                        `;
                        chatContent += createMessageBubble('assistant', queryHtml, '', 'bi-search');
                    }
                                ...
                                if (trial.search_query) {
                                    ...
                                    if (resultsCount > 0) {
                                        ...
                                        const resultsHtml = `
                                            <div class="d-flex align-items-center justify-content-between">
                                                ...
                                                <button class="btn btn-sm btn-outline-success rounded-pill px-3 view-search-results-btn" data-results="${resultsJson}">
                                    ...
                                    if (trial.final_answer_instruction) {
                                        ...
                                        if (sourcesFound) {
                                            chatContent += createMessageBubble('user', `
                                            ...
                                            chatContent += createMessageBubble('user', faInstr);
                                        }
                                    }
                                }
                                ...
                                chatContent += createMessageBubble('assistant', answerBody, '', 'bi-chat-left-dots');
                                ...
                                if (trial.status === 'completed' && trial.feedback) {
                                    ...
                                    chatContent += `
                                        <div class="d-flex justify-content-center mt-2 mb-2 fade-in">
