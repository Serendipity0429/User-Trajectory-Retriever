document.addEventListener('DOMContentLoaded', function() {
    ...
    startBtn.addEventListener('click', function() {
        ...
        const pipelineType = document.getElementById('pipeline-mode-session').value;
    ...
    function loadRun(groupId) {
        ...
        fetch(`/benchmark/api/multi_turn/load_rag_run/${groupId}/`)
            .then(res => res.json())
            .then(data => {
                if (data.error) {
    ...
    function runQAPipeline() {
        ...
        const pipelineType = document.getElementById('pipeline-mode-pipeline').value;
        ...
        // Generate a temporary ID for tracking stop requests
        ...
        // Extract reform strategy from pipelineType (e.g. 'rag_multi_turn_reform')
        let reformStrategy = 'no_reform';
        if (pipelineType.includes('reform')) {
            reformStrategy = 'reform';
        }
        if (pipelineType.includes('no_reform')) {
            reformStrategy = 'no_reform';
        }
            ...
            BenchmarkUtils.processStreamedResponse(
                ...
                (data) => {
                    if (data.error) {
                        ...
                        // Optionally add a visual indicator for the failed item
                         if (data.question) {
                             const errorMsg = `Error processing question: ${data.question.substring(0, 30)}...`;
                             // We could append this to a log or a specific error container
                         }
                        return;
                    }
                    ...
                    // data corresponds to one completed session result
    ...
    function stopPipeline() {
        ...
        if (pipelineController.id) {
            ...
            // We need to send reformulation strategy for proper stop key
            const pipelineType = document.getElementById('pipeline-mode-pipeline').value;
            let reformStrategy = 'no_reform';
            if (pipelineType.includes('reform')) reformStrategy = 'reform';
            if (pipelineType.includes('no_reform')) reformStrategy = 'no_reform';

            fetch(window.benchmarkUrls.stopRagMultiTurnPipeline, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': csrfToken
                },
                body: JSON.stringify({ pipeline_id: pipelineController.id, reformulation_strategy: reformStrategy }),
                keepalive: true
