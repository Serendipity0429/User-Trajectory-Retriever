    ...
    def handle(self, *args, **kwargs):
        with transaction.atomic():
            ...
            for task in active_tasks:
                self.stdout.write(f'Deleting task {task.id} for user {task.user.username}...')
                task.delete()

        self.stdout.write(self.style.SUCCESS(f'Successfully deleted {num_deleted} active tasks.'))
