...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        longest_task = Task.objects.filter(
            active=False, 
            cancelled=False
        ).annotate(
            duration=Cast(F('end_timestamp') - F('start_timestamp'), DurationField())
        ...
        if longest_task:
            ...
            self.stdout.write(f"Duration: {duration_minutes:.2f} minutes ({duration_seconds:.2f} seconds)")
            self.stdout.write("="*30)
        else:
            self.stdout.write(self.style.WARNING('No completed tasks found to analyze.'))
