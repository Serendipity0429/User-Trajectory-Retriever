...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if existing_datasets.exists():
            ...
            self.stdout.write(
                f"NQ-open {dataset_name} dataset already exists, deleting it."
            )
            existing_datasets.delete()
            # self.stdout.write(f"NQ-open {dataset_name} dataset already exists, skipping.")
            # return

        dataset = TaskDataset.objects.create(name=dataset_name, path=dataset_path)
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Dataset NQ-open {dataset_name} loaded successfully.")
        )
