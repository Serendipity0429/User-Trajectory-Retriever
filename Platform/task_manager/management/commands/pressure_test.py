...
class Command(BaseCommand):
    ...
    def _generate_pseudo_data(self, payload_size_kb):
        ...
        for i in range(num_events):
            event_list.append({
                ...
                "screenX": random.randint(0, 1920), "screenY": random.randint(0, 1080),
                "clientX": random.randint(0, 1920), "clientY": random.randint(0, 1080),
                "tag": "DIV", "content": "Some content", "hierachy": "BODY/DIV/DIV", "related_info": {}
            })
        ...
        return {
            'url': url,
            'title': 'Test Page - ' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=6)),
            'referrer': referrer,
    ...
    def populate_db(self, User, username, password, count):
        ...
        self.stdout.write(self.style.SUCCESS(f"Populating database with {count} records..."))
        ...
        entry, _ = TaskDatasetEntry.objects.get_or_create(
            belong_dataset=dataset, question='Pressure Test', answer=json.dumps(['Done'])
        )
        task, _ = Task.objects.update_or_create(user=user, active=True, defaults={'content': entry})
        ...
        self.stdout.write(self.style.SUCCESS(f"Successfully populated database for user {username}."))
    ...
    def setup_and_run_test(self, User, username, password, options):
        ...
        Task.objects.update_or_create(user=user, active=True, defaults={'content': entry})
        ...
        try:
            ...
            self.stdout.write(f"Running End-to-End Pressure Test with {options['payload_size']}KB payload...")
            self.stdout.write(self.style.SUCCESS('=' * 50))
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS('Pressure test completed successfully.'))
            except FileNotFoundError:
                self.stdout.write(self.style.ERROR("Error: 'ab' command not found. Please install Apache Bench."))
