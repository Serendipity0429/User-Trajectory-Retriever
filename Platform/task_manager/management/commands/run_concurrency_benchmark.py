...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['clean']:
            ...
            self.stdout.write(self.style.SUCCESS("Benchmark data cleaned successfully."))
            return
        self._main_benchmark_logic(options)

    def _run_test(self, concurrency, payload_size, num_requests=15, description="Testing", suppress_result_print=False):
        self.stdout.write(f"  - {description}: C={concurrency}, P={payload_size}KB...", ending='')
        self.stdout.flush()
        ...
        try:
            ...
            failed_count = int(re.search(r"Failed requests:\s+(\d+)", output).group(1)) if re.search(r"Failed requests:\s+(\d+)", output) else 0
            ...
            if time_match:
                ...
                self.stdout.write(" UNEXPECTED OUTPUT" if not suppress_result_print else " FAIL")
                return -4.0, num_requests

        except Exception as e:
            self.stdout.write(f" FAILED (error: {e})" if not suppress_result_print else " FAIL")
            return None, None
    ...
    def _cleanup_test_data(self):
        ...
        if 'sqlite3' in engine:
            ...
            if os.path.exists(db_path):
                ...
                subprocess.run(['sqlite3', db_path, 'VACUUM;'], capture_output=True, text=True)
    ...
    def _draw_graphs(self, all_results, db_sizes, concurrency_levels, payload_sizes, num_requests, output_dir):
        ...
        for db_size in db_sizes:
            ...
            if perf_results:
                ...
                for payload in sorted(payload_sizes):
                    ...
                    if not all(np.isnan(ys)):
                        ax2d_perf.plot(xs, ys, marker='o', linestyle='-', label=f'Payload {payload / 1024:.2f}MB')
                
                ax2d_perf.set_xscale('log', base=2); ax2d_perf.set_yscale('log')
                ax2d_perf.set_xlabel('Concurrency (log scale)'); ax2d_perf.set_ylabel('Avg. Response Time (ms, log scale)')
                ax2d_perf.set_title(f'Performance Profile (Log-Log)\nDB Size: {db_size} records'); ax2d_perf.legend()
                ...
                if not np.all(np.isnan(Z)):
                    ax3d_perf.plot_surface(np.log2(X), np.log2(Y), np.log10(Z), cmap='magma', edgecolor='k', linewidth=0.5, alpha=0.9)
                    ax3d_perf.set_xlabel('Concurrency (log2 scale)'); ax3d_perf.set_ylabel('Payload Size (KB, log2 scale)'); ax3d_perf.set_zlabel('Response Time (ms, log10 scale)')
                    ax3d_perf.set_title(f'3D Performance Surface (Log-Log-Log)\nDB Size: {db_size} records')
            ...
            if error_results:
                ...
                for payload in sorted(payload_sizes):
                    ...
                    if not all(np.isnan(ys)):
                        ax2d_err.plot(xs, ys, marker='x', linestyle='--', label=f'Payload {payload / 1024:.2f}MB')

                ax2d_err.set_xscale('log', base=2); ax2d_err.set_yscale('symlog', linthresh=1)
                ax2d_err.set_xlabel('Concurrency (log scale)'); ax2d_err.set_ylabel('Error Rate (%, symlog scale)')
                ax2d_err.set_title(f'Error Rate Profile (Log-Symlog)\nDB Size: {db_size} records'); ax2d_err.legend()
                ...
                if not np.all(np.isnan(Z_err)):
                    ax3d_err.plot_surface(np.log2(X), np.log2(Y), Z_err, cmap='cividis', edgecolor='k', linewidth=0.5, alpha=0.9)
                    ax3d_err.set_xlabel('Concurrency (log2 scale)'); ax3d_err.set_ylabel('Payload Size (KB, log2 scale)'); ax3d_err.set_zlabel('Error Rate (%)')
                    ax3d_err.set_title(f'3D Error Rate Surface (Log-Log-Linear)\nDB Size: {db_size} records')
    ...
    def _main_benchmark_logic(self, options):
        scales = {
            'small': {
                "db_sizes": np.logspace(6, 10, num=2, base=2, dtype=int).tolist(),      # 64, 1k
                "concurrency": np.logspace(2, 4, num=3, base=2, dtype=int).tolist(),   # 4, 8, 16
                "payloads_kb": np.logspace(3, 9, num=3, base=2, dtype=int).tolist(),   # 8k, 64k, 512k
                "requests": 32,
            },
            'large': {
                "db_sizes": np.logspace(6, 18, num=4, base=2, dtype=int).tolist(),     # 4 points from 64 to 256k
                "concurrency": np.logspace(1, 6, num=8, base=2, dtype=int).tolist(),   # 8 points from 2 to 64
                "payloads_kb": np.logspace(1, 14, num=8, base=2, dtype=int).tolist(),  # 8 points from 2KB to 16MB
                "requests": 64,
            }
        }
        ...
        num_requests_per_test = options['requests'] if options['requests'] is not None else config['requests']

        max_concurrency = max(concurrency_levels)
        if num_requests_per_test < max_concurrency:
            self.stdout.write(f"Warning: Requests ({num_requests_per_test}) < max concurrency ({max_concurrency}). Adjusting to {max_concurrency}.")
            num_requests_per_test = max_concurrency
        
        all_results = []
        self.stdout.write(self.style.SUCCESS(f"====== Starting Benchmark (Scale: {options['scale'].upper()}) ======"))
        ...
        for db_size in db_sizes:
            ...
            for payload in payload_sizes_kb:
                for concurrency in concurrency_levels:
                    ...
                    self._run_test(
                        warmup_concurrency, warmup_payload, warmup_requests, 
                        description="Warming up", suppress_result_print=True
                    )
                    
                    response_time, failed_requests = self._run_test(concurrency, payload, num_requests_per_test)
            ...
            self._draw_graphs(all_results, [db_size], concurrency_levels, payload_sizes_kb, num_requests_per_test, options['output_dir'])

        self._print_summary_table(all_results, db_sizes, concurrency_levels, payload_sizes_kb)
        
        self.stdout.write(self.style.SUCCESS("\n====== Benchmark Finished ======"))
        self._cleanup_test_data()

    def _print_summary_table(self, all_results, db_sizes, concurrency_levels, payload_sizes_kb):
        def format_time(res):
        ...
        self.stdout.write(f"|------------|--------------|" + "|".join(["-"*20 for _ in concurrency_levels]) + "|")
        ...
        for db_size in db_sizes:
            ...
            if db_size != db_sizes[-1]: self.stdout.write(f"|------------|--------------|" + "|".join(["-"*20 for _ in concurrency_levels]) + "|")