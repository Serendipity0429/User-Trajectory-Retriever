...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.SUCCESS(f"Task Statistics Report (By {group_by.title().replace('_', ' ')})"))
        self.stdout.write(self.style.SUCCESS("="*30))
    ...
    def stats_by_question(self):
        ...
        if not questions.exists():
            self.stdout.write(self.style.WARNING("No questions with associated tasks found."))
            return
        ...
        for question in questions:
            ...
            if tasks_for_question.exists():
                ...
                self.display_comprehensive_stats(tasks_for_question, f"Question {question.id}", output_dir)
            else:
                self.stdout.write(self.style.WARNING("No finished tasks for this question."))
    ...
    def stats_by_task_id(self):
        ...
        for task in tasks:
            ...
            if not task.cancelled:
                duration_min = (task.end_timestamp - task.start_timestamp).total_seconds() / 60
                ...
                for trial in trials:
                    trial_duration_min = (trial.end_timestamp - trial.start_timestamp).total_seconds() / 60
                    self.stdout.write(f"  - Trial {trial.num_trial}: {trial_duration_min:.2f} minutes, Justifications: {Justification.objects.filter(belong_task_trial=trial).count()}")
    ...
    def question_summary_stats(self):
        ...
        for question in questions:
            ...
            if correct_count > 0:
                avg_duration = np.mean([(t.end_timestamp - t.start_timestamp).total_seconds() / 60 for t in correct_tasks])
        ...
        self.display_stats("Average Task Completion Time Across Questions (minutes)", avg_durations)
        self.display_stats("Average Number of Trials Across Questions", avg_trials, is_time=False)
        self.display_stats("Success Rate Across Questions (%)", success_rates, is_time=False)

        self.generate_histograms(avg_durations, "avg_task_completion_time_across_questions", output_dir)
        self.generate_histograms(avg_trials, "avg_num_trials_across_questions", output_dir)
        self.generate_histograms(success_rates, "success_rate_across_questions", output_dir, xlabel="Success Rate (%)")
    ...
    def display_comprehensive_stats(self, tasks_queryset, title_prefix, output_dir):
        ...
        task_durations = [(task.end_timestamp - task.start_timestamp).total_seconds() / 60 for task in correct_tasks_q]
        ...
        trial_durations = [(trial.end_timestamp - trial.start_timestamp).total_seconds() / 60 for trial in trials if trial.end_timestamp and trial.start_timestamp]
        self.display_stats("Task Trial Time (minutes)", trial_durations)

        # 4. Histograms
        self.generate_histograms(task_durations, "task_completion_time_minutes", output_dir)
        ...
        self.display_stats("Number of Trials per Correct Task", num_trials, is_time=False)

        justifications_per_trial = [Justification.objects.filter(belong_task_trial=trial).count() for trial in trials]
        self.display_stats("Justifications per Trial", justifications_per_trial, is_time=False)
    ...
    def generate_histograms(self, data, name, output_dir, xlabel=None):
        ...
        self.stdout.write(self.style.SUCCESS(f"Generated log-transformed histogram and saved as {log_filename}"))
        plt.close()
