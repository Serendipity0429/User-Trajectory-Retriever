...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--concurrent', action='store_true', help='Generate and validate answers concurrently.')
        parser.add_argument('--source_path', type=str, help='Path to the source QA dataset.')
        parser.add_argument('--output_path', type=str, help='(Optional) Path to save the detailed results jsonl file.')
    ...
    def handle(self, *args, **options):
        if options['concurrent']:
            if not options['source_path']:
                self.stderr.write(self.style.ERROR("For --concurrent, you must provide --source_path."))
                return
            asyncio.run(self.run_concurrently(options['source_path'], options['output_path'], options['workers'], options['retries']))
        else:
            self.stdout.write(self.style.WARNING("This command now only supports --concurrent mode."))

    async def run_concurrently(self, source_path, output_path, num_workers, max_retries):
        self.stdout.write(f"Starting concurrent generation and validation with {num_workers} workers and {max_retries} retries.")
        
        try:
            with open(source_path, 'r') as f:
                questions = [json.loads(line) for line in f]
        except FileNotFoundError:
            self.stderr.write(self.style.ERROR(f"Dataset file not found at: {source_path}"))
            return
        ...
        tasks = [asyncio.create_task(self.worker(f"worker-{i}", client, model, queue, results, output_file, file_lock, max_retries)) for i in range(num_workers)]
        ...
        if output_file:
            output_file.close()
            self.stdout.write(self.style.SUCCESS(f"Detailed results saved to {output_path}"))
    ...
    async def worker(self, name, client, model, queue, results, output_file, file_lock, max_retries):
        while True:
            try:
                ...
                self.stdout.write(f"[{name}] Processing question #{item_idx}: {question}")

                # 1. Generate Answer
                generated_answer = await self.generate_llm_answer(client, model, question, max_retries)
                ...
                # 2. Rule-based check
                is_correct_rule = self.check_answer_rule_sync(question, authentic_answers, generated_answer)
                
                # 3. LLM-as-a-judge check
                is_correct_llm = await self.check_answer_llm_async(client, model, question, authentic_answers, generated_answer, max_retries)

                # 4. Compare and log
                if is_correct_rule == is_correct_llm:
                    results['agreements'] += 1
                    self.stdout.write(self.style.SUCCESS(f"-> AGREEMENT on question #{item_idx} (Rule: {is_correct_rule}, LLM: {is_correct_llm})"))
                else:
                    results['disagreements'] += 1
                    self.stdout.write(self.style.WARNING(f"-> DISAGREEMENT on question #{item_idx} (Rule: {is_correct_rule}, LLM: {is_correct_llm})"))
                ...
                self.stderr.write(self.style.ERROR(f"[{name}] Error processing item: {e}"))
                results['errors'] += 1
                queue.task_done()
...
Answer:"""
        for attempt in range(max_retries + 1):
            try:
                ...
                if completion.choices and completion.choices[0].message and completion.choices[0].message.content:
                    ...
                    self.stderr.write(self.style.ERROR(f"Error: LLM returned empty content for question '{question_text}'"))
                    # This is considered a final failure for this attempt, but we can still retry
            except Exception as e:
                self.stderr.write(self.style.WARNING(f"Attempt {attempt + 1}/{max_retries + 1} failed for '{question_text}': {e}"))
        ...
        self.stderr.write(self.style.ERROR(f"All {max_retries + 1} attempts failed for question '{question_text}'."))
        return None
    ...
    async def check_answer_llm_async(self, client, model, question, authentic_answers, user_answer, max_retries):
...
Is the user's answer correct?"""
        for attempt in range(max_retries + 1):
            try:
                ...
                self.stderr.write(self.style.WARNING(f"LLM Judge Attempt {attempt + 1}/{max_retries + 1} failed for '{question}': {e}"))
        ...
        self.stderr.write(self.style.ERROR(f"LLM Judge failed for question '{question}' after {max_retries + 1} attempts."))
        return False
