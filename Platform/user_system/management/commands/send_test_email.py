...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully found or created user with email: {email}"
                )
            )

            reset_request = ResetPasswordRequest.objects.create(user=user)
            self.stdout.write(
                self.style.SUCCESS(f"Generated reset token: {reset_request.token}")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully sent a test password reset email to {email}"
                )
            )
